...
def read_data(dataset, dataset_path, offset_=None, shape_=None, base=True):
	print("base:", base, " / offset: ", offset_, " shape ", shape_)
...
def get_searcher_path(dataset_basedir, program, dataset, distance, num_split, split):
	searcher_dir = dataset_basedir + program + '_searcher_' + distance + '/' + dataset + '/Split_' + str(num_split) + '/'
	searcher_path = searcher_dir + dataset + '_searcher_' + str(num_split)+'_'+str(split)
	
	os.makedirs(searcher_dir, exist_ok=True)
	os.makedirs(searcher_path, exist_ok=True)
#searcher_dir = "./" + args.program + '_searcher_' + args.metric + '/' + args.dataset + '/Split_' + str(args.num_split) + '/'
...
def get_result_filename(count=None, definition_args=None,
                        query_arguments=None):
    d = ['results']
    d.append(args.dataset)
    d.append(str(args.topk))
    d.append(args.program)
    data = definition_args.arguments + query_arguments
    d.append(re.sub(r'\W+', '_', json.dumps(data, sort_keys=True)).strip('_'))
    return os.path.join(*d)

def store_results(dataset, count, definition, query_arguments, attrs, results,
                  batch):
    fn = get_result_filename(dataset, count, definition_args, query_arguments) + '.hdf5'
...
def get_queries(dataset, dataset_basedir):
	if "sift1m" in dataset:
